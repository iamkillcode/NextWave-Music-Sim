...
export const updateNextTubeDaily = onSchedule({
	...
	while (true) {
		...
		const batch = db.batch();
			...
			// Optional quality-based multipliers (uses player's embedded songs to avoid extra reads)
			let qualityViewsMult = 1.0;
			try {
				const songs: any[] = Array.isArray((player as any).songs) ? (player as any).songs : [];
				const songId = (data.songId || "").toString();
				if (songId) {
					const s = songs.find((x) => x && (x.id || "").toString() === songId);
					if (s) {
						let q = numOr(s.quality, 50);
						if (Number.isFinite(s.recordingQuality)) {
							const rq = Number(s.recordingQuality);
							if (Number.isFinite(rq)) q = Math.round((numOr(s.quality, 50) + rq) / 2);
						}
						const coef = Math.max(0, Math.min(0.05, getParam('nexQualityCoef', 0.01)));
						qualityViewsMult = Math.max(0.5, Math.min(1.5, 1 + ((q - 50) * coef)));
					}
				}
			} catch {}
			...
			// Earnings (cents) with small RPM quality bias
			let adjRpm = rpmCents;
			try {
				const songs: any[] = Array.isArray((player as any).songs) ? (player as any).songs : [];
				const songId = (data.songId || "").toString();
				if (songId) {
					...
					if (s) {
						...
						const bias = Math.max(0.8, Math.min(1.2, 1 + ((q - 50) * getParam('nexRPMQualityBias', 0.002))));
						adjRpm = Math.round(adjRpm * bias);
						adjRpm = Math.max(RPM_MIN, Math.min(RPM_MAX, adjRpm));
					}
				}
			} catch {}

			const earningsCents = isMonetized ? Math.floor((adjRpm * viewsToday) / 1000) : 0;
...
// Bridge selected legacy JS exports for incremental Gen 2 deployment.
// This lets us deploy only a few functions at a time without pulling in the entire JS monolith.
// The JS file already defines v2 triggers; we simply re-export them here so the CLI can discover them.
// IMPORTANT: keep this list small to avoid long cold-start analysis.
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore - CommonJS interop for legacy index.js
import * as legacy from "../index.js";

// Certifications: small batch
// - Lists user's album eligibility for next certification tier
export const listAlbumCertificationEligibility = legacy.listAlbumCertificationEligibility as any;
// - Submits an album for certification (server-validated)
export const submitAlbumForCertification = legacy.submitAlbumForCertification as any;
// - Admin retroactive migration to award missing song certifications
export const runCertificationsMigrationAdmin = legacy.runCertificationsMigrationAdmin as any;

// Core gameplay callables: Deploy batch by batch
// - Secure song creation (server-validated)
export const secureSongCreation = legacy.secureSongCreation as any;
// - Secure album release (server-validated)
export const secureReleaseAlbum = legacy.secureReleaseAlbum as any;
// - Check admin status (auth helper)
export const checkAdminStatus = legacy.checkAdminStatus as any;
// - Send gift to player (admin utility)
export const sendGiftToPlayer = legacy.sendGiftToPlayer as any;
// - Trigger daily update (admin/testing)
export const triggerDailyUpdate = legacy.triggerDailyUpdate as any;
