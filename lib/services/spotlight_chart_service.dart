...
class SpotlightChartService {
  ...
  Future<List<Map<String, dynamic>>> getSpotlightHot100({int limit = 100}) async {
    try {
  ...
  Future<int?> getSpotlight200Position(String songTitle, String artistId) async {
    try {
      final chart = await getSpotlight200(limit: 200);
      
      for (int i = 0; i < chart.length; i++) {
        if (chart[i]['title'] == songTitle && chart[i]['artistId'] == artistId) {
          return i + 1;
        }
      }
  ...
  Future<Map<String, int?>> getSpotlightPositions(String songTitle, String artistId, bool isAlbum) async {
    final positions = <String, int?>{};

    if (isAlbum) {
      positions['spotlight200'] = await getSpotlight200Position(songTitle, artistId);
    } else {
