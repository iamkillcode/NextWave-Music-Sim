...
class StreamGrowthService {
  ...
  int calculateDailyStreamGrowth({
    ...
    final daysSinceRelease = currentGameDate.difference(song.releasedDate!).inDays;
    ...
    final tunifyStreams = (loyalStreams + discoveryStreams + viralStreams + casualStreams) * 0.85;
    final mapleStreams = (loyalStreams + discoveryStreams + viralStreams + casualStreams) * 0.65;
  ...
  Map<String, int> calculateRegionalStreamDistribution({
    ...
    final totalFanbase = regionalFanbase.values.fold(0, (sum, fans) => sum + fans);
    ...
    if (remainingStreams > 0) {
      distribution[currentRegion] = (distribution[currentRegion] ?? 0) + remainingStreams;
    }
  ...
  int _calculateDiscoveryStreams({
    ...
    if (daysSinceRelease == 0) {
      final releaseHype = (artistStats.fanbase * 0.3 * song.finalQuality / 100).round();
      return (releaseHype * (1.5 + _random.nextDouble())).round();
    }
    
    // Week 1: High discovery (algorithm boost)
    if (daysSinceRelease <= 7) {
      final weekOneDiscovery = (artistStats.fanbase * 0.2 * song.viralityScore).round();
      final dayDecay = 1.0 - (daysSinceRelease / 7.0 * 0.4); // 40% decay over week
      return (weekOneDiscovery * dayDecay).round();
    }
    
    // Week 2-4: Medium discovery
    if (daysSinceRelease <= 30) {
      final monthOneDiscovery = (artistStats.fanbase * 0.1 * song.viralityScore).round();
      final weekDecay = 1.0 - ((daysSinceRelease - 7) / 23.0 * 0.5); // Further 50% decay
      return (monthOneDiscovery * weekDecay).round();
    }
    
    // Month 2-3: Low but consistent discovery
    if (daysSinceRelease <= 90) {
      final lateDiscovery = (artistStats.fanbase * 0.05 * song.viralityScore).round();
      return (lateDiscovery * (0.5 + _random.nextDouble() * 0.5)).round();
    }
    
    // After 3 months: Long tail (very low, quality matters more)
    final longTail = (artistStats.fanbase * 0.02 * (song.finalQuality / 100)).round();
  ...
  int _calculateViralStreams({
    ...
    if (_random.nextDouble() < viralChance * 0.1) {
      ...
      final spikeMultiplier = 2.0 + (_random.nextDouble() * 5.0); // 2x to 7x spike
      final baseViralStreams = (song.streams * 0.05).round(); // 5% of current streams
  ...
  int _calculateCasualFanStreams({
    ...
    final casualFans = (artistStats.fanbase - artistStats.loyalFanbase).clamp(0, double.infinity).toInt();
    
    // Only a fraction of casual fans stream on any given day
    final engagementRate = (songQuality / 100.0) * 0.2; // Max 20% of casual fans
  ...
  double calculateViralityScore({
    ...
    final fanbaseMultiplier = 0.9 + (artistFanbase / 10000.0 * 0.2).clamp(0.0, 0.2);
    ...
    final virality = qualityFactor * fameMultiplier * fanbaseMultiplier * luckFactor;
  ...
  int calculateLoyalFanbaseGrowth({
    ...
    if (songQuality >= 70) {
      ...
      final conversionRate = 0.05 + ((songQuality - 70) / 30.0 * 0.05); // 5% to 10%
      final newLoyal = (gap * conversionRate).round();
      return newLoyal;
    } else if (songQuality >= 50) {
      // Good songs convert 2-5%
      final gap = totalFanbase - currentLoyalFanbase;
      final conversionRate = 0.02 + ((songQuality - 50) / 20.0 * 0.03); // 2% to 5%
      final newLoyal = (gap * conversionRate).round();
      return newLoyal;
    }
